...
import { createProposalDraftAction } from "@/lib/proposal-actions";
import {
...
export function ProposalBuilder({
  ...
  const currentStepDefinition = proposalBuilderSteps[getStepIndex(currentStep)];
  ...
  function handleContinue() {
    ...
    const nextStep = proposalBuilderSteps[currentIndex + 1];
  ...
  function handleBack() {
    const previousStep = proposalBuilderSteps[getStepIndex(currentStep) - 1];
    ...
    <form action={createProposalDraftAction}>
      ...
      <section className="proposal-builder-layout">
        <div className="builder-panel">
          <nav aria-label="Etapas do builder de proposta" className="builder-stepper">
            {proposalBuilderSteps.map((step, index) => {
              const isActive = step.id === currentStep;
              const isUnlocked = isStepUnlocked(step.id, hasLead, selectedItems.length);
              const isComplete = index < getStepIndex(currentStep);
              ...
              return (
                ...
                >
                  ...
                  <span className="builder-step-meta">
          ...
          <div className="builder-mobile-summary" aria-live="polite">
            <div className="builder-mobile-summary-head">
              <span>{currentStepDefinition?.label ?? "Builder de proposta"}</span>
              <strong>{formatCurrencyFromCents(totalCents)}</strong>
            </div>
            <div className="builder-mobile-summary-meta">
              <span>{activeLead ? activeLead.company : "Lead pendente"}</span>
              <span>
          ...
          <article className="surface-card">
            <div className="section-head">
              <p className="eyebrow">{getCurrentStepEyebrow(currentStep)}</p>
              <h2>{getCurrentStepHeading(currentStep)}</h2>
            </div>
            ...
            <div className="page-stack">
              {initialLeadId ? (
                <div className="builder-inline-note">
                  ...
                  <p>
                    Este builder foi aberto a partir de um registro de lead. Você pode manter essa
                    seleção ou trocar para um rascunho rápido se a oportunidade tiver mudado.
                  </p>
                </div>
              ) : null}
              ...
              {leadMode === "existing" ? (
                ...
                <div className="page-stack">
                  <div className="lead-form-grid">
                    <div className="two-column-grid">
                      <label className="field">
                        ...
                        <input
                          onChange={(event) => updateLeadDraft("fullName", event.target.value)}
                      ...
                      <label className="field">
                        <span>Empresa</span>
                        <input
                          onChange={(event) => updateLeadDraft("company", event.target.value)}
                      ...
                      <label className="field">
                        <span>Origem</span>
                        <select
                          onChange={(event) => updateLeadDraft("source", event.target.value)}
                      ...
                      <label className="field">
                        ...
                        <select
                          onChange={(event) => updateLeadDraft("assignedPartner", event.target.value)}
                    ...
                    <label className="field">
                      ...
                      <textarea
                        onChange={(event) => updateLeadDraft("notes", event.target.value)}
                  ...
                  <div className="builder-inline-note">
                    ...
                    <p>
                      Esta etapa mantém a captura do lead leve para geração de proposta. Se você
                      precisar de um registro separado depois, use a{" "}
            ...
            <div className="page-stack">
              <div className="builder-toolbar">
                <p className="section-copy">
                  Somente usuários internos autenticados podem ver o catálogo completo da Onebridge
                  e seus preços. Selecione os serviços que devem compor esta proposta.
                </p>
                        ...
                        >
                          ...
                          <div className="catalog-service-head">
                            <div className="catalog-service-copy">
                              <span className="catalog-service-kicker">{service.internalCode}</span>
                            ...
                            <div className="catalog-service-pricing">
                              ...
                              <span>
                                ...
                                {getUnitLabel(service.unitLabel)}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="catalog-service-tags">
                            ...
                            <StatusPill tone="neutral">
                              {service.allowsVariableQuantity ? "Quantidade variável" : "Quantidade fixa"}
                            </StatusPill>
                            <StatusPill tone={service.isActive ? "success" : "warning"}>
                              {service.isActive ? "Ativo" : "Inativo"}
                            </StatusPill>
                          </div>

                          {service.specificClause || service.submissionNotes ? (
                            <div className="catalog-service-notes">
                              {service.specificClause ? (
                                <p>
                                  <strong>Cláusula específica:</strong> {service.specificClause}
                                </p>
                              ) : null}

                              {service.submissionNotes ? (
                                <p>
                                  <strong>Observações para envio:</strong> {service.submissionNotes}
                                </p>
                              ) : null}
                            </div>
                          ) : null}
          ...
          {currentStep === "quantities" ? (
            <div className="page-stack">
              <p className="section-copy">
                As quantidades só podem ser editadas quando o catálogo permite variação de forma
                explícita. Toda a matemática permanece em centavos inteiros para manter o snapshot
                final estável.
              </p>
            ...
            <div className="page-stack">
              <div className="review-grid">
                ...
                <div className="snapshot-note">
                  ...
                  <p>
                    O Franklin hoje suporta apenas acesso de administrador e sócio. Até que as
                    permissões de desconto sejam modeladas explicitamente, o total da proposta
                    permanece sem descontos.
                  </p>
                </div>
              </div>
              ...
              {snapshotPreview ? (
                <div className="review-grid">
                  <div className="snapshot-note">
                    <strong>Campos do snapshot que serão salvos na criação do rascunho</strong>
                    <ul className="feature-list">
                      <li>Empresa, contato, email, telefone, origem e observações do cliente</li>
                      <li>Nome do serviço, nome original, tipo de cobrança, unidade, quantidade e preço</li>
                      <li>Subtotal da proposta, desconto zero e total em centavos inteiros</li>
                    </ul>
                  </div>
                  ...
                  <div className="snapshot-note">
                    ...
                    <div className="data-list">
                      ...
                      <div className="data-row">
                        ...
                        <strong>{formatCurrencyFromCents(snapshotPreview.totalCents)}</strong>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="builder-actions">
            <div className="builder-actions-group">
              ...
              {currentStep !== "review" ? (
                <button
                  ...
                  disabled={!canContinue(currentStep, hasLead, selectedItems.length)}
            ...
            <p className="builder-actions-note">
              {getContinueHint(currentStep, hasLead, selectedItems.length)}
            </p>
          </div>
          </article>
        </div>
        ...
        <article className="surface-card">
          ...
          <div className="page-stack">
            ...
            <div className="discount-disabled">
              ...
              <p>Eles permanecem bloqueados até que permissões comerciais por perfil sejam modeladas.</p>
            </div>
          </div>
        </article>
        ...
        <article className="surface-card notice-panel">
          ...
          <ul className="feature-list">
            ...
            <li>O rascunho é criado primeiro e o token público seguro só é emitido quando a proposta é enviada.</li>
            <li>A geração de PDF continua adiada, mas o aceite já tem um gatilho durável preparado.</li>
          </ul>
        </article>
        </aside>
      </section>
...
function getCurrentStepEyebrow(step: ProposalBuilderStepId): string {
  switch (step) {
    case "lead":
      return "Etapa 1";
    case "services":
...
function getCurrentStepHeading(step: ProposalBuilderStepId): string {
  switch (step) {
    case "lead":
      return "Anexe o lead que deve receber esta proposta";
    case "services":
      return "Selecione apenas os serviços que pertencem a este rascunho";
    case "quantities":
      return "Valide quantidades, preços unitários e subtotais por item";
    case "review":
      return "Revise o snapshot da proposta antes da criação do rascunho durável";
  }
}
...
function CreateDraftButton({ disabled }: { disabled: boolean }) {
  ...
  return (
    <button className="button-primary" disabled={disabled || pending} type="submit">
      {pending ? "Criando rascunho..." : "Criar rascunho de proposta"}
    </button>
  );
}
...
function getContinueHint(
  ...
  switch (currentStep) {
    ...
    case "review":
      return "A prévia do snapshot está pronta. Envio e PDF continuam fora do escopo desta fase.";
  }
}
